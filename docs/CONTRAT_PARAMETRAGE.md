...
## 8. Lot E — le prêt paramétrable, et la démonstration sur ta configuration

*Étape E.0. Écrit le 22/09/2026, avant toute ligne de code. Prolonge le contrat
du lot C : ce qui est écrit ici est ce que le lecteur, les calculs et le
générateur feront, et rien d'autre ne sera deviné en silence.*

Les décisions E1 à E9 ont été arbitrées le 21/09/2026 (§3 de
`plan/19_PLAN_ACTION_LOT_E.md`) et validées le 22/09/2026. Elles ne se rouvrent
pas.

### E1 — Le prêt se déclare par quatre valeurs

Le bloc Prêt de la feuille `Paramètres` accepte quatre valeurs :
`Prêt — montant`, `Prêt — date de début`, `Prêt — taux annuel`,
`Prêt — durée (mois)`. La **mensualité** et la courbe de projection sont
**calculées** — `M = P·r / (1 − (1 + r)^−n)`, avec `r = taux annuel / 12` et
`n = durée`. C'est l'inverse du calcul actuel, qui recevait la mensualité et
cherchait le taux par approximation.

Tout ou rien, comme le bloc actuel : les quatre valeurs présentes ensemble, ou
le bloc entier est ignoré et l'écran Prêt disparaît (§5.1 du format). Un taux
annuel de 0 % donne une mensualité de `P / n`, sans division par zéro.

Le graphique « Historique mensuel — capital & intérêts » **ne change pas** : il
reste fondé sur les transactions de nature `pret-capital` et `pret-interets`.

### E2 — Les deux blocs Prêt restent lisibles

L'ancien bloc (`Prêt — mensualité`, `Prêt — nombre d'échéances`) et le nouveau
(`Prêt — taux annuel`, `Prêt — durée (mois)`) sont **tous deux lus**. Un fichier
écrit avec l'ancien bloc continue de donner les mêmes chiffres : c'est la
contrainte du §8.1 du format — une v2.1 se reconnaît par des lignes nouvelles,
jamais par une ligne détournée.

Les deux blocs présents à la fois doivent donner la **même mensualité à 1 €
près**. Sinon le bloc Prêt est **refusé**, en affichant les deux valeurs et en
disant laquelle corriger. L'outil ne choisit pas à la place de la personne — la
même règle que pour la chaîne de comptes liés (D3a) et le `Sens répercuté`
(§4.7).

### E3 — Le prêt inventé disparaît du code

Les trois constantes `LOAN_PRINCIPAL = 180 000`, `LOAN_PAYMENT = 893,6`,
`LOAN_TERMS = 240` sont **supprimées**. C'était la dernière valeur par défaut
chiffrée du dashboard.

Sans bloc Prêt déclaré, l'écran Prêt ne montre **que** ce qui vient des
transactions : l'historique mensuel réel du capital et des intérêts. Plus de
projection, plus de capital restant dû, plus d'échéancier — et l'écran le dit :
« déclarez votre prêt (montant, date de début, taux, durée) pour voir la
projection ». Plus de 180 000 € sortis de nulle part.

L'indicateur `parametresDeRepli` disparaît avec les constantes : il n'a plus de
repli à signaler.

### E4 — La démonstration reprend la STRUCTURE de ta configuration, pas ses noms

*Révisé le 25/09/2026 (lot G).* La première version de E4 faisait entrer les
vrais noms d'établissements dans la démo. Décision annulée, pour raison de
sécurité et de protection des données personnelles.

Ce qui reste repris de ta configuration : la **structure** — nombre de comptes,
natures, taux de participation, comptes liés, classes. Les **noms** sont
génériques : Banque A, Banque B, Banque C, Titres-restaurant, Appli partagée,
Assurance-vie, Cagnotte partagée.

Aucun nom réel — établissement, employeur, marchand, ville, identité — n'entre
dans le dépôt. La liste de ces noms n'y entre pas non plus : elle vit hors du
dépôt (fichier privé ignoré par Git, secret GitHub `LISTE_NOIRE` en CI). Voir
§10, journal v5.

### E5 — Tout le reste est inventé

Montants, dates, libellés, villes, détails, sous-catégories, salaires **et
employeurs** : **tous inventés**, graine fixe, aucun commerçant réel, aucune
ville réelle, aucun employeur réel. Un vrai libellé dirait où tu habites et où
tu fais tes courses ; un vrai employeur dirait pour qui tu travailles — c'est
précisément ce qui ne doit pas fuiter.

### E6 — Ta configuration entre dans le générateur à la main

La structure de E4 est **recopiée** dans `scripts/generate-demo-data.mjs`, sous des noms génériques. Le
générateur ne lit **jamais** ton classeur — ni pour générer, ni à aucun autre
moment. Sinon la démo dépendrait d'un fichier réel présent sur une seule
machine, et cesserait d'être reproductible ailleurs.

La règle 2 en tête du générateur, qui interdit aujourd'hui tout nom réel, est
**réécrite** : la structure de configuration est recopiée sous des noms
génériques ; les montants, dates et libellés restent inventés.

### E7 — Un garde-fou de reproductibilité, avant d'ouvrir la liste noire

`npm run check` **relance le générateur** et compare son résultat, **octet pour
octet**, avec les fichiers de `public/data/`. Un fichier que le générateur ne
sait pas refaire à l'identique est **bloqué**, quels que soient les noms qu'il
porte.

C'est ce qui remplace le second rôle de la liste noire. Jusqu'ici, elle était le
dernier rempart contre la publication d'un vrai export de tes données. Une liste
de noms ne voit que les noms qu'on lui a donnés. Le garde-fou le rattrape : un vrai export n'est pas ce que le générateur
produit, donc il ne concorde pas octet pour octet.

Ce contrôle est posé **avant** l'ouverture de la liste noire (E.3 avant E.4 du
plan), et il doit **savoir échouer** : prouvé en déposant un `transactions.json`
qui n'est pas celui du générateur, et en le voyant bloqué. Un rapprochement vert
qui ne mesure rien est le piège nommé du lot.

### E8 — Les salaires inventés le disent

Des salaires chiffrés : un lecteur pourrait croire que ce sont les tiens. L'écran Salaire de la démonstration porte la
mention **« montants inventés »**, visible — pas un tiret muet en pied de page.

### E9 — Le classeur modèle porte les mêmes noms que la démonstration

Le classeur modèle téléchargeable — celui qui sert aux visiteurs à préparer leur
propre fichier — porte les **noms génériques** de comptes, types et catégories,
**cohérents avec la démonstration affichée**. Ses montants, dates et libellés
restent inventés (E5), comme le reste de la démo.

### La règle E, comme la règle C

Quand l'outil ne sait pas, il le **dit**. Un prêt écrit en dur, un zéro, une
liste figée : autant de mensonges qui ne se voient pas. Le lot E retire le
dernier prêt inventé (E3) et pose, avant d'ouvrir la liste noire, le garde-fou
qui empêche un vrai export de passer pour une démo (E7).

---

## 9. Lot F — le prêt calculé depuis sa date, et le vrai modèle v5

*Étape F.0. Écrit le 24/09/2026, avant toute ligne de code. Plan de
référence : `plan/21_PLAN_ACTION_LOT_F.md`. Décisions F4 à F12 et O1 à O6 validées le
24/09/2026.*

### Ce que F.0 a mesuré

- **SheetJS gratuit n'écrit pas de listes déroulantes** : à la place du bloc
  `dataValidations`, son code ne porte qu'un commentaire vide.
- **`exceljs` 4.4.0, licence MIT** : il écrit listes déroulantes et formules
  avec leur valeur en cache. Deux fabrications donnent des `.xlsx` différents
  (heure des entrées du zip), mais un **contenu décompressé identique** quand
  les dates du document sont figées. Dates libres → la comparaison échoue sur
  `docProps/core.xml` : elle sait échouer.
- **L'application donne déjà les bons soldes** sur un classeur réel au format
  v5 : ses liquidités concordent au centime avec celles que le classeur
  calcule lui-même. Un écart vu dans ce classeur venait de **la formule de sa
  feuille Visualisation**, pas du code (voir F11). Chiffres : `plan/`, jamais
  ici.

### F1 à F3 — déjà prises (plan, §3)

L'échéancier se calcule depuis la déclaration ; l'échéance courante se situe
par le **dernier mois des données**, jamais par l'horloge. Les transactions
**vérifient** la déclaration. Le modèle est un vrai `.xlsx`, publié.

### F4 — Un seul `.xlsx` autorisé, par son chemin exact

`public/modele/Budget_modele.xlsx`, jamais l'extension. `npm run check` refait
le modèle et compare le **contenu décompressé**, dates du document figées.
Prouvé en échec avant d'être cru au vert.

### F5 — `exceljs`, outil de développement, script seulement

Il n'entre pas dans le build.

### F6 — Le modèle : structure du v5, données de la démonstration

Le script ne lit **aucun** classeur. Il calcule lui-même la valeur en cache des
colonnes que l'application lit. La feuille `Visualisation` est recalculée par
Excel à l'ouverture (`fullCalcOnLoad`). Importer le modèle redonne les chiffres
de la démo.

### F7 — `Sortie Epargne` déclaré comme compte

Import accepté, avertissement nommé : c'est un type depuis le format v2, la
ligne de compte est ignorée.

### F8 — Prêt sans date : position « estimée »

L'ancien calcul, par les intérêts, reste. L'écran dit que la position est
estimée et invite à déclarer la date.

### F9 — Le bouton télécharge le fichier publié

`construireClasseurModele` quitte le worker ; `modeleExcel.ts` disparaît, avec
son exemption D5.

### F10 — On saisit le montant brut, le fichier calcule le montant réel

*Remplace O3.* Dans la feuille `Transactions` du modèle :

- la colonne D s'appelle **`Montant brut`** : c'est le montant payé, en entier,
  et la seule colonne de montant que l'utilisateur saisit ;
- une colonne calculée **`Montant`** donne le montant réel :
  `ARRONDI(Montant brut × Participation du compte ; 2)`. Le taux est lu dans
  `Paramètres`, colonne `Participation`. Case vide = 100 % ;
- la `Visualisation` additionne `Montant`, jamais `Montant brut`.

Exemple : 100 € sur `Banque A - Part commune` à 50 % → le fichier
affiche 50, l'application compte 50.

**Côté application, D1 ne change pas.** `Montant brut` rempli → l'application
applique elle-même le taux (même arrondi au centime). Un fichier qui n'a que
`Montant` est lu comme avant. Deux ajouts :

- **concordance** : quand les deux colonnes sont remplies, l'application compare
  son calcul avec le `Montant` du fichier ; s'ils diffèrent, elle le dit,
  chiffré (un taux changé sans recalcul du fichier) ;
- **avertissement** : un compte déclaré avec une participation, mais aucun
  `Montant brut` rempli → le taux ne s'applique pas, et l'outil le dit.

### F11 — Un remboursement est un débit annulé

**La règle existe déjà dans l'application** (lot C.7) : un crédit dont le type
porte la nature `remboursement` réduit la dépense d'origine, et, sur un compte
lié, se répercute comme un **débit négatif**. Le montant reste positif (§4.1).

*Amendé le 24/09/2026 (F.3).* Un remboursement s'écrit sur le **type de la
dépense qu'il annule** (« courses » + `Crédit` dans `Sens (si différent)`), et
réduit donc **sa catégorie**. Règle de l'application : un crédit est un
remboursement si son type porte la nature `remboursement`, **ou** si son type
a une `Classe par défaut` (c'est un type de dépense). Le type
`Remboursement` reste accepté. Exemple : 40 € de courses, 15 € remboursés →
25 € d'Alimentation.

Ce que F11 ajoute aussi : **les formules du modèle suivent la même règle.** La
formule de liquidité mesurée ne lisait qu'un sens par compte lié : sur un
compte lié qui répercute ses débits, le remboursement n'était plus vu ; sur
un compte lié qui répercute ses crédits, il était pris pour un virement venu
du compte principal.

**Critère, posé par l'auteur** : la `Visualisation` du modèle, appliquée à
ses données, donne **les mêmes résultats** que celle de son propre classeur.
Vérifié cellule par cellule ; les chiffres restent dans `plan/`.

### F12 — Une ligne prévisionnelle dont la date est passée

*Validée le 24/09/2026.*

Mesuré : dans le classeur de l'auteur, une ligne `Prévisionnel` **compte** dès
que sa date est passée (colonne `Dépassé Date`). Le format v5 et
l'application l'**excluent** toujours. Une échéance de prêt échue était ainsi
comptée d'un côté, pas de l'autre.

**Règle** : une ligne prévisionnelle compte si sa date est **antérieure ou
égale à la `Fin de relevé`** déclarée dans `Paramètres` — jamais l'horloge de la
machine, pour que la démo reste identique d'un jour à l'autre (F1). Sans
`Fin de relevé` déclarée, elle reste exclue, et l'aperçu d'import le dit,
chiffré.

### O1 à O6 — tranchées le 24/09/2026

- **O1** — Tolérance du contrôle F2 : 2 € sur les intérêts d'un mois, 5 € sur
  la mensualité. Réglée au F.2, en agrégats.
...
- **O5** — `Prêt — date de début` devient **obligatoire** pour la forme
  ...
  *Constat : le code actuel accepte la forme « taux » sans date, contrairement
  à E1.*
- **O6** — La démo passe en forme « taux » : 180 000 €, 2022-10, 1,80 %,
  240 mois. Mensualité 893,64 € au lieu de 893,60 € : les KPI du prêt bougent
  de quelques euros, écart expliqué ligne par ligne au F.1.

### La règle F, comme les règles C et E

Quand l'outil ne sait pas, il le **dit**. Un taux qui ne s'applique pas, un
remboursement pris pour un virement, une date devinée : autant de mensonges
...
| v2 | 22/09/2026 | Lot E.0. Nouvelle section §8 : décisions E1 à E9 (prêt paramétrable par quatre valeurs, deux blocs Prêt lisibles, prêt inventé supprimé, démo sur la vraie configuration, garde-fou de reproductibilité avant l'ouverture de la liste noire). Aucun code touché. |
| v3 | 22/09/2026 | Lot E.4. E4 et E5 amendés : seuls les 7 établissements/services financiers entrent dans la démo (retirés de la liste noire) ; employeurs **inventés** et marchands **bloqués** (ni la localisation ni les employeurs ne doivent fuiter). |
| v4 | 24/09/2026 | Lot F.0. Nouvelle section §9 : décisions F1 à F11 et O1 à O6 (prêt calculé depuis sa date, modèle `.xlsx` publié sous garde-fou, montant brut saisi et montant réel calculé, remboursement = débit annulé dans les formules du modèle). F12 (ligne prévisionnelle échue : comptée jusqu’à la Fin de relevé) validée le même jour. Le journal devient le §10. Aucun code touché. |
| v5 | 25/09/2026 | Lot G. E4 révisé : plus aucun nom réel d'établissement dans la démo, noms génériques (Banque A/B/C, Titres-restaurant, Appli partagée, Assurance-vie, Cagnotte partagée). La liste noire sort du dépôt (fichier privé ignoré + secret CI) ; le contrôle ne cite plus jamais le texte trouvé. Historique Git repris à zéro. |
