...
import { validerConfig, lireTaux } from "@/services/validerConfig";
import { mensualiteDepuisTaux, calculerPret } from "@/calculs/calculPret";
...
export interface TransactionLue {
  ...
  estBrut?: true;
  /**
   * F10 — la ligne portait AUSSI un `Montant` (le fichier l'a calculé). Gardé
   * le temps de comparer avec le calcul de l'outil, puis retiré.
   */
  montantFichier?: number;
  /** F10 — où signaler un écart de concordance. Retiré avec `montantFichier`. */
  origine?: { feuille: string; ligne: number };
}
...
export interface ParametresLus {
  ...
  pret: {
    montant: number; mensualite: number; echeances: number;
    premiereEcheance?: string; tauxAnnuel?: number; dateDebut?: string;
  } | null;
 ...
 * sont CONSERVÉS mais signalés, avec leurs deux emplacements : deux opérations
 * identiques le même jour existent vraiment (deux cafés, deux tickets), et les
 * effacer retirait de vraies dépenses. Un import qui duplique un fichier entier,
 * lui, produit des avertissements en masse qui le rendent visible.
 */
function lireTransactions(
  wb: XLSX.WorkBook,
  col: Collecteur,
  compteurs: CompteursImport,
  finReleve: string | null
) {
  const feuilles = feuillesTransactions(wb);
  const out: TransactionLue[] = [];
  const vues = new Map<string, string>();
  // F12 — lignes prévisionnelles écartées faute de `Fin de relevé`.
  const prevSansFin = { lignes: 0, montant: 0 };
  ...
  for (const nom of feuilles) {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // ── F12 : une ligne prévisionnelle ÉCHUE compte ───────────────────
      // Échue = datée au plus tard à la `Fin de relevé` déclarée. Jamais
      // l'horloge de la machine : le même fichier donne les mêmes chiffres
      // tous les jours. Une ligne échue est lue comme une ligne constatée, avec
      // les mêmes contrôles ; les autres restent de côté, comme avant.
      if (normaliserCle(lire(r, "previsionnel")) === "x") {
        const d = lireDate(lire(r, "date"));
        const echue = finReleve !== null && d !== null && d <= finReleve;
        if (!echue) {
          compteurs.ignorees++;
          compteurs.lignesLues--;
          if (finReleve === null) {
            prevSansFin.lignes++;
            const m = lireNombre(lire(r, "montantBrut")) ?? lireNombre(lire(r, "montant"));
            if (m !== null && m > 0) prevSansFin.montant = arrondir(prevSansFin.montant + m);
          }
          continue;
        }
      }
      ...
      if (deja) {
        // C.7 — décision alignée sur l'ancien tableur : une opération identique
        // répétée le même jour (deux tickets, deux cafés à 0,40 €) est RÉELLE.
        // On la CONSERVE et on signale, au lieu de la rejeter en silence. Un
        // import de fichier dupliqué reste visible : il produit l'avertissement
        // pour chacune de ses lignes.
        col.ajouter({
          gravite: "avertissement", feuille: nom, ligne, colonne: COLONNES_TX.date,
          message:
            `Doublon strict de ${deja} — même date, même compte, même type, même montant, ` +
            `même libellé. La ligne est CONSERVÉE (deux opérations identiques le même jour ` +
            `arrivent). Si c'est un import en double, retirez-la.`,
        });
      }
      vues.set(cle, `${nom} ligne ${ligne}`);

      // F10 — les deux colonnes remplies : le `Montant` du fichier est gardé
      // pour être comparé au calcul de l'outil, une fois les taux connus.
      const montantFichier = aUnBrut ? lireNombre(brutMontant) : null;

      out.push({
        label, compte, type, date, montant, cat1, cat2,
        ...(aUnBrut ? { estBrut: true as const } : {}),
        ...(montantFichier !== null
          ? { montantFichier, origine: { feuille: nom, ligne } }
          : {}),
  ...
  if (prevSansFin.lignes > 0) {
    col.ajouter({
      gravite: "avertissement", feuille: feuilles[0] ?? "Transactions",
      colonne: COLONNES_TX.previsionnel,
      message:
        `${prevSansFin.lignes} ligne(s) prévisionnelle(s) écartée(s) ` +
        `(${prevSansFin.montant.toFixed(2).replace(".", ",")} €). Sans « Fin de relevé » ` +
        `dans la feuille Paramètres, l'outil ne sait pas lesquelles sont échues. ` +
        `Déclarez-la pour compter celles dont la date est passée.`,
    });
  }
...
/**
 * F12 — la `Fin de relevé`, lue sans rien signaler. Même règle que
 * `lireParametres` : les deux bornes lisibles, et dans l'ordre, ou rien.
 */
function lireFinDeReleve(wb: XLSX.WorkBook): string | null {
  ...
  for (const [cle, valeur] of tableauParEnTete(grille, "Paramètre")) {
    params.set(normNom(cle), valeur);
  }
  const debut = lireDate(params.get(normNom("Début de relevé")) ?? null);
  const fin = lireDate(params.get(normNom("Fin de relevé")) ?? null);
  return debut && fin && fin >= debut ? fin : null;
}
...
function lireParametres(wb: XLSX.WorkBook, col: Collecteur): ParametresLus {
  ...
  // ── Prêt : tout ou rien, en deux formes (E.1) ────────────────────────
  // Forme « mensualité » (historique) : montant + mensualité + échéances.
  // Forme « taux » (v2.1) : montant + taux annuel + durée (mois) — la
  // mensualité est calculée. Les deux présentes doivent concorder à 1 €
  // près (E2), sinon le bloc entier est refusé.
  ...
  const tauxAnnuel = lireTaux(lireParam("Prêt - taux annuel")?.v ?? null);
  const dureeMois = lireNombre(lireParam("Prêt - durée (mois)")?.v ?? null);

  // ── La date du prêt — lot F.1, décision O5 ───────────────────────────
  // `date de début` et `première échéance` sont deux noms du MÊME mois :
  // celui de l'échéance n° 1. Une valeur écrite mais illisible est dite,
  // jamais devinée ; deux valeurs qui se contredisent refusent le bloc.
  const brutDebut = lireParam("Prêt - date de début")?.v ?? null;
  const brutPremiere = lireParam("Prêt - première échéance")?.v ?? null;
  const ecrit = (v: unknown) => v != null && nettoyerTexte(v) !== "";
  const moisDebut = lireMois(brutDebut);
  const moisPremiere = lireMois(brutPremiere);
  let dateIllisible = false;
  for (const [libelle, brut, mois] of [
    ["date de début", brutDebut, moisDebut],
    ["première échéance", brutPremiere, moisPremiere],
  ] as const) {
    if (ecrit(brut) && !mois) {
      dateIllisible = true;
      col.ajouter({
        ...
        message:
          `Prêt — ${libelle} illisible : « ${nettoyerTexte(brut)} ». Écrivez un ` +
          `mois (AAAA-MM) ou une date. Le bloc Prêt entier est ignoré.`,
      });
    }
  }
  const datesContradictoires = !!moisDebut && !!moisPremiere && moisDebut !== moisPremiere;
  if (datesContradictoires) {
    col.ajouter({
      gravite: "avertissement", feuille: nom, colonne: "Valeur",
      message:
        `Bloc Prêt incohérent — « date de début » dit ${moisDebut}, « première ` +
        `échéance » dit ${moisPremiere}. Ce sont deux noms du même mois (celui de ` +
        `l'échéance n° 1) : le bloc entier est refusé. Gardez-en une seule.`,
    });
  }
  const moisPret = moisDebut ?? moisPremiere;
  const dateDebut = moisPret ?? "";
  const premiere = moisPremiere ?? "";

  const formeTaux =
    montant !== null && montant > 0 &&
    tauxAnnuel !== null &&
    dureeMois !== null && dureeMois > 0;
  const formeMensualite =
    montant !== null && montant > 0 &&
    mensualite !== null && mensualite > 0 &&
    echeances !== null && echeances > 0;

  let pret: ParametresLus["pret"] = null;

  if (dateIllisible || datesContradictoires) {
    // Déjà dit plus haut : le bloc est ignoré, rien n'est deviné.
  } else if (formeTaux && !moisPret) {
    // O5 — la forme « taux » calcule l'échéancier depuis sa date (F1). Sans
    // date, elle produirait une position inventée : refusée, et dit.
    col.ajouter({
      gravite: "avertissement", feuille: nom, colonne: "Valeur",
      message:
        "Bloc Prêt incomplet — la forme « taux » demande aussi « Prêt — date de " +
        "début » (le mois de la première échéance). Sans elle, l'outil ne peut pas " +
        "situer votre prêt : le bloc entier est ignoré.",
    });
  } else if (formeTaux) {
    const mensualiteCalc = mensualiteDepuisTaux(montant!, tauxAnnuel!, dureeMois!);
    if (formeMensualite && Math.abs(mensualiteCalc - mensualite!) > 1) {
      col.ajouter({
        gravite: "avertissement", feuille: nom, colonne: "Valeur",
        message:
          `Bloc Prêt incohérent — la forme « taux » donne une mensualité de ` +
          `${mensualiteCalc.toFixed(2)} €, la forme « mensualité » en déclare ` +
          `${mensualite!.toFixed(2)} €. Écart supérieur à 1 € : le bloc entier est ` +
          `refusé. Corrigez l'une des deux, ou n'en gardez qu'une.`,
      });
    } else {
      pret = {
        montant: montant!, mensualite: mensualiteCalc, echeances: dureeMois!,
        tauxAnnuel: tauxAnnuel!,
        ...(dateDebut ? { dateDebut } : {}),
        ...(premiere ? { premiereEcheance: premiere } : {}),
      };
    }
  } else if (formeMensualite) {
    pret = {
      montant: montant!, mensualite: mensualite!, echeances: echeances!,
      // F1 : la date, quand elle est donnée, situe aussi un prêt de forme
      // « mensualité ». Sans elle, la position est estimée (F8).
      ...(dateDebut ? { dateDebut } : {}),
      ...(premiere ? { premiereEcheance: premiere } : {}),
    };
  } else {
    const nbChamps =
      [montant, mensualite, echeances, tauxAnnuel, dureeMois].filter((x) => x !== null).length +
      (dateDebut ? 1 : 0);
    if (nbChamps > 0) {
      col.ajouter({
        gravite: "avertissement", feuille: nom, colonne: "Valeur",
        message:
          "Bloc Prêt incomplet — il faut soit (montant, date de début, taux annuel, " +
          "durée), soit (montant, mensualité, nombre d'échéances). Le bloc entier est ignoré : un " +
          "montant réel accolé à une valeur par défaut produirait un échéancier " +
          "crédible et faux.",
      });
    }
  }
 ...
 *
 * Le partage lui-même n'émet AUCUNE anomalie, et c'est délibéré. Dire « 2
 * lignes ont été partagées selon leur taux » est une INFORMATION : rien ne
 * cloche, l'outil a fait ce que le fichier demandait. L'aperçu d'import le
 * dit dans le bloc « Configuration lue ».
 *
 * F10 (lot F.2bis) — DEUX avertissements, parce que là quelque chose cloche :
 *  - le `Montant` que le fichier a calculé ne correspond pas au calcul de
 *    l'outil (un taux changé sans recalcul du fichier) ;
 *  - un compte déclaré avec un taux n'a aucune ligne en `Montant brut` : le
 *    taux ne s'appliquera jamais, en silence.
 */
function appliquerParticipation(
  transactions: TransactionLue[],
  config: BudgetConfig,
  feuille: string,
  col: Collecteur
  ...
  // F10 — concordance : un écart par COMPTE, chiffré, jamais un par ligne
  // (la leçon du plafond de 200 anomalies, lot C.2).
  const discordances = new Map<
    string,
    { lignes: number; premiere: { feuille: string; ligne: number }; calcule: number; lu: number }
  >();

  for (const t of transactions) {
    if (t.estBrut) {
      t.montant = arrondir(t.montant * (taux.get(normaliserCle(t.compte)) ?? 1));
      if (t.montantFichier !== undefined && Math.abs(t.montant - t.montantFichier) > 0.005) {
        const d = discordances.get(t.compte);
        if (d) d.lignes++;
        else discordances.set(t.compte, {
          lignes: 1, premiere: t.origine!, calcule: t.montant, lu: t.montantFichier,
        });
      }
    }
    delete t.montantFichier;
    delete t.origine;
  }

  const eur = (x: number) => `${x.toFixed(2).replace(".", ",")} €`;
  for (const [compte, d] of discordances) {
    col.ajouter({
      gravite: "avertissement", feuille: d.premiere.feuille, ligne: d.premiere.ligne,
      colonne: COLONNES_TX.montant,
      message:
        `« ${compte} » : sur ${d.lignes} ligne(s), le « Montant » du fichier ne ` +
        `correspond pas au « Montant brut » × le taux de participation déclaré. ` +
        `Première : ${eur(d.lu)} dans le fichier, ${eur(d.calcule)} calculé par l'outil. ` +
        `L'outil garde SON calcul. Le taux a-t-il changé sans que le fichier soit recalculé ?`,
    });
  }

  // F10 — un taux déclaré qui ne s'appliquera jamais : le compte n'a aucune
  // ligne en `Montant brut`, ses montants sont pris tels quels.
  const parCompte = new Map<string, { lignes: number; avecBrut: number }>();
  for (const t of transactions) {
    const k = normaliserCle(t.compte);
    const c = parCompte.get(k) ?? { lignes: 0, avecBrut: 0 };
    c.lignes++;
    if (t.estBrut) c.avecBrut++;
    parCompte.set(k, c);
  }
  for (const c of config.comptes) {
    if (typeof c.participation !== "number" || c.participation === 1) continue;
    const vu = parCompte.get(normaliserCle(c.libelle));
    if (!vu || vu.avecBrut > 0) continue;
    col.ajouter({
      gravite: "avertissement", feuille, colonne: COLONNES_TX.montantBrut,
      message:
        `« ${c.libelle} » est déclaré à ${Math.round(c.participation * 100)} % de ` +
        `participation, mais aucune de ses ${vu.lignes} ligne(s) n'a de « Montant brut ». ` +
        `Le taux ne s'applique pas : ses montants sont pris tels quels. Saisissez le ` +
        `montant payé en entier dans « Montant brut » pour que l'outil applique le taux.`,
    });
  }
}
...
/**
 * F.2 — contrôle de cohérence du prêt, dans l'aperçu d'import.
 * Un avertissement, jamais un rejet : l'import a lieu, et l'écart est dit.
 */
function signalerPretIncoherent(
  transactions: TransactionLue[],
  parametres: ParametresLus,
  col: Collecteur
): void {
  const p = parametres.pret;
  if (!p) return;
  const config: Config = {
    init: {},
    demo: false,
    ...(parametres.config.estVide ? {} : { parametrage: parametres.config }),
    pret: {
      montant: p.montant, mensualite: p.mensualite, echeances: p.echeances,
      ...(p.tauxAnnuel != null ? { taux_annuel: p.tauxAnnuel } : {}),
      ...(p.dateDebut ? { date_debut: p.dateDebut } : {}),
    },
  };
  const tx: Transaction[] = transactions.map((t) => ({ ...t, monthKey: t.date.slice(0, 7) }));
  const { controle } = calculerPret(tx, config, construireRegles(config));
  if (controle.etat !== "diverge") return;
  col.ajouter({
    gravite: "avertissement", feuille: "Paramètres", colonne: "Valeur",
...
export function lireClasseurPublic(wb: XLSX.WorkBook): RapportImport {
  ...
  // F12 — la `Fin de relevé` est lue AVANT les transactions, en silence : elle
  // dit quelles lignes prévisionnelles sont échues. Ses anomalies éventuelles
  // sont émises plus bas, par `lireParametres`, une seule fois.
  const { transactions, feuilles } = lireTransactions(wb, col, compteurs, lireFinDeReleve(wb));
  ...
  appliquerParticipation(transactions, parametres.config, feuilles[0] ?? "Transactions", col);
  ...
  // F.2 — l'aperçu d'import dit, lui aussi, quand les transactions ne
  // concordent pas avec le prêt déclaré. Même calcul que l'écran Prêt : une
  // seule source, sinon les deux messages divergent le jour où l'un change.
  signalerPretIncoherent(transactions, parametres, col);
