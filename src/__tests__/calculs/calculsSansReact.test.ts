...
describe("les calculs s'appellent sans React", () => {
  ...
  it("calculerPret dit qu'il n'a pas de données quand il n'y a pas d'échéance", () => {
    const p = calculerPret(TX, null, R);
    expect(p.hasData).toBe(false);
    // Sans prêt déclaré par la source, les paramètres viennent du repli — et
    // le calcul le DIT, au lieu de présenter un échéancier inventé.
    expect(p.parametresDeRepli).toBe(true);
  });
});
