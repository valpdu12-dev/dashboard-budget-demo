...
describe("aucune donnée personnelle dans les fichiers publiés", () => {
  // Lot G : la liste vit hors du dépôt. Absente, ce test ÉCHOUE — sauf
  // décision explicite LISTE_NOIRE_FACULTATIVE=1.
  const { motifs, erreurs } = lireListeNoire(resolve(__dirname, "../../.."));

  it.skipIf(motifs === null && listeFacultative())(
    "ne contient aucun établissement, employeur, marchand, lieu ni nom réel",
    () => {
      expect(erreurs).toEqual([]);
      for (const nom of ["transactions.json", "salary.json", "config.json", "budgets.json"]) {
        const texte = readFileSync(resolve(__dirname, "../../../public/data", nom), "utf8");
        // Un numéro de motif, jamais le texte trouvé.
        expect(trouverInterdit(texte, motifs ?? []), nom).toBeNull();
      }
    }
  );
});
