...
import { lireListeNoire, trouverInterdit, listeFacultative } from "../../../scripts/listeNoire.mjs";
...
describe("aucun nom réel ne peut entrer par une couleur", () => {
  // Lot G : la liste des noms interdits ne vit plus dans le dépôt. Elle est
  // lue hors du dépôt (secret CI ou fichier privé), et son absence fait
  // échouer ce test — sauf décision explicite LISTE_NOIRE_FACULTATIVE=1.
  const { motifs, erreurs } = lireListeNoire(resolve(__dirname, "../../.."));

  it.skipIf(motifs === null && listeFacultative())(
    "les libellés déclarés par la démonstration ne portent aucun nom interdit",
    () => {
      // Une couleur se déclare à côté d'un LIBELLÉ : c'est par ce chemin
      // qu'un nom réel est entré une fois.
      expect(erreurs).toEqual([]);
      const tout = [...COMPTE_LIBELLES, ...ORGANISMES_UTILISES].join("\n");
      // Le test rend un numéro de motif, jamais le texte trouvé.
      expect(trouverInterdit(tout, motifs ?? [])).toBeNull();
    }
  );
});
