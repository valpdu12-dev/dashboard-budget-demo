...
// Le prêt déclaré dans la config des tests : 180 000 € à 893,60 €/mois sur
// 240 échéances. Depuis E.3, sans ce bloc il n'y a plus de repli — donc les
// tests qui vérifient des KPIs le déclarent explicitement.
const PRET = { montant: 180000, mensualite: 893.6, echeances: 240 };

/**
 * Transactions de prêt FICTIVES, calculées depuis le tableau d'amortissement
 * du prêt ci-dessus : échéances #44 (avr.) et #45 (mai 2026). Aucun chiffre ne
 * provient d'un prêt réel.
 */
...
describe("useMortgageData — KPIs", () => {
  beforeEach(() => {
    useDataStore.getState().setData(makeMortgageTxs(), makeSalaryData(), makeConfig({ pret: PRET }));
  });
  ...
  it("avancement cohérent et capital remboursé = principal − restant", () => {
    ...
    expect(kpis.capitalRembourse).toBeCloseTo(kpis.principal - kpis.capitalRestant, 2);
  });
