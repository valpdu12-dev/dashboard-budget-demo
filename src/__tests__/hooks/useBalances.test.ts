...
describe("useBalances — règles Banque A - Courant", () => {
  ...
  it("Appli partagée - Part commune Crédit → aucun impact sur Banque A - Courant", () => {
    // F11 (24/09/2026) : un crédit sur « CB », type de dépense, est désormais
    // un remboursement. Ce test vise un crédit ordinaire : type sans classe.
    const tx = [makeTx({ compte: "Appli partagée - Part commune", type: "Transfert Banque A vers Banque B", dc: "Crédit", montant: 200, monthKey: "2025-01" })];
    const { result } = renderHook(() => useBalances(tx, ["2025-01"], INIT_ZERO));
    expect(result.current.balancesByMonth["2025-01"]["Banque A - Courant"]).toBe(0);
  });
  ...
  it("Banque B Crédit (non Virement ext.) → -montant sur Banque A - Courant", () => {
    // F11 (24/09/2026) : un crédit sur « CB », type de dépense, est désormais
    // un remboursement. Ce test vise un crédit ordinaire : type sans classe.
    const tx = [makeTx({ compte: "Banque B - Compte joint", dc: "Crédit", montant: 400, type: "Transfert Banque A vers Banque B", monthKey: "2025-01" })];
    const { result } = renderHook(() => useBalances(tx, ["2025-01"], INIT_ZERO));
    // Impact sur Banque A - Courant : -400 et sur Banque B : +400
  ...
  it("Banque B Crédit Virement extérieur → PAS d'impact sur Banque A - Courant", () => {
  ...
  it("Banque C Crédit (non Virement ext.) → -montant sur Banque A - Courant", () => {
  ...
  it("Banque C Crédit Virement extérieur → PAS d'impact sur Banque A - Courant", () => {
...
// ═════════════════════════════════════════════════════════════════════════
// 3. Règles simples : comptes à solde propre (Banque B joint / Banque C / Titres-restaurant)
// ═════════════════════════════════════════════════════════════════════════
describe("useBalances — comptes simples", () => {
  it("Banque B Débit → -montant sur Banque B", () => {
  ...
  it("Banque C Débit → -montant sur Banque C", () => {
...
describe("useBalances — scénario intégré", () => {
  it("gère un flux réaliste multi-comptes sur 2 mois", () => {
    const tx: Transaction[] = [
      // Janvier : salaire Banque A - Courant, dépense CB Banque A - Courant, virement vers Banque B
      makeTx({ compte: "Banque A - Courant", dc: "Crédit", montant: 2500, monthKey: "2025-01" }),
      makeTx({ compte: "Banque A - Courant", dc: "Débit", montant: 800, monthKey: "2025-01" }),
      makeTx({ compte: "Banque B - Compte joint", dc: "Crédit", montant: 600, type: "Virement", monthKey: "2025-01" }),
      // → Banque A - Courant = 5000+2500-800-600 = 6100, Banque B = 2000+600 = 2600
