...
describe("les quatre jeux de test", () => {
  it("jeu complet — paie et prêt", () => {
    const r = rubriquesDisponibles([...makeTransactions(), echeance(), interets()], PAIE, CONFIG_AVEC_PRET);
    expect(r).toEqual({ paie: true, pret: true });
  });

  it("jeu sans paie — le prêt reste", () => {
    const r = rubriquesDisponibles([...makeTransactions(), echeance()], SANS_PAIE, CONFIG_AVEC_PRET);
    expect(r).toEqual({ paie: false, pret: true });
  });

  it("jeu sans prêt — la paie reste", () => {
    const r = rubriquesDisponibles(makeTransactions(), PAIE, CONFIG_SANS_PRET);
    expect(r).toEqual({ paie: true, pret: false });
  });

  it("jeu sans paie ni prêt", () => {
    const r = rubriquesDisponibles(makeTransactions(), SANS_PAIE, CONFIG_SANS_PRET);
    expect(r).toEqual({ paie: false, pret: false });
  });
});

describe("ce qui fait exister un prêt", () => {
  it("des échéances dans les transactions suffisent, sans rien de déclaré", () => {
    expect(rubriquesDisponibles([echeance()], SANS_PAIE, CONFIG_SANS_PRET).pret).toBe(true);
  });

  it("un prêt déclaré suffit, même sans aucune échéance passée", () => {
    // Un prêt qui commence le mois prochain est un prêt.
    expect(rubriquesDisponibles([], SANS_PAIE, CONFIG_AVEC_PRET).pret).toBe(true);
  });
  ...
  it("un bloc prêt incomplet ne fait PAS exister de prêt", () => {
    ...
    expect(rubriquesDisponibles([], SANS_PAIE, bancal).pret).toBe(false);
  });
  ...
  it("ne se laisse pas décider par la période affichée", () => {
    ...
    expect(rubriquesDisponibles(complet, PAIE, CONFIG_SANS_PRET).pret).toBe(true);
  });
});

describe("cas limites", () => {
  it("aucune donnée : aucune rubrique", () => {
    expect(rubriquesDisponibles([], null, null)).toEqual({ paie: false, pret: false });
  });

  it("une paie vide ne compte pas comme une paie", () => {
    expect(rubriquesDisponibles([], makeSalaryData([]), null).paie).toBe(false);
  });
});
...
describe("le jeu de démonstration publié", () => {
  ...
  it("fait exister la paie ET le prêt", () => {
    ...
    expect(rubriquesDisponibles(tx, salaire, config)).toEqual({ paie: true, pret: true });
  });
});
