...
function makeSavingsTxs() {
  return [
    // Entrées épargne (type Épargne Banque A = EPARGNE_TYPES)
    makeTx({ date: "2025-01-05", type: "Épargne Banque A", dc: "Débit", montant: 300, monthKey: "2025-01", compte: "Banque A - Courant" }),
    makeTx({ date: "2025-02-05", type: "Épargne Banque A", dc: "Débit", montant: 300, monthKey: "2025-02", compte: "Banque A - Courant" }),
    makeTx({ date: "2025-03-05", type: "Épargne Banque A", dc: "Débit", montant: 300, monthKey: "2025-03", compte: "Banque A - Courant" }),
    // Sortie épargne
...
describe("useSavingsData — KPIs", () => {
  ...
  it("retourne ratio=0 quand recettes=0", () => {
    const txs = [makeTx({ date: "2025-03-05", type: "Épargne Banque A", dc: "Débit", montant: 200, monthKey: "2025-03", compte: "Banque A - Courant" })];
...
describe("useSavingsData — donutData", () => {
  it("agrège par type", () => {
    const txs = [
      makeTx({ date: "2025-03-01", type: "Épargne Banque A",       dc: "Débit", montant: 200, monthKey: "2025-03", compte: "Banque A - Courant" }),
      makeTx({ date: "2025-03-02", type: "Épargne Banque B", dc: "Débit", montant: 150, monthKey: "2025-03", compte: "Banque A - Courant" }),
    ];
    useDataStore.getState().setData(txs, makeSalaryData(), makeConfig());
    const { result } = renderHook(() => useSavingsData());
    expect(result.current.donutData).toHaveLength(2);
    const ca = result.current.donutData.find((d) => d.name === "Épargne Banque A");
    expect(ca?.value).toBe(200);
  });

  it("filtre par selEpMonth si défini", () => {
    const txs = [
      makeTx({ date: "2025-02-01", type: "Épargne Banque A", dc: "Débit", montant: 100, monthKey: "2025-02", compte: "Banque A - Courant" }),
      makeTx({ date: "2025-03-01", type: "Épargne Banque A", dc: "Débit", montant: 200, monthKey: "2025-03", compte: "Banque A - Courant" }),
    ];
    useDataStore.getState().setData(txs, makeSalaryData(), makeConfig());
