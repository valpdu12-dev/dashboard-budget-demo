// ── Le calcul des KPIs de l'en-tête, sorti de React ──────────────────────
//
// Lot C.3. Déplacé depuis `hooks/useKPIs.ts`, sans qu'une seule règle change.
//
...
import { montantDepense, estRecette } from "@/calculs/mouvements";
...
export function calculerKPIs(
  ...
  const isTransfer = (t: Transaction) => regles.aNature(t.type, "transfert-interne");
  ...
  const depCur = curTx.reduce((s, t) => s + montantDepense(t, regles), 0);
  const depPrev = prevTx.reduce((s, t) => s + montantDepense(t, regles), 0);
  const recCur = curTx
    .filter((t) => estRecette(t, regles))
