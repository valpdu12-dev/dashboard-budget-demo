...
import { montantDepense, toucheDepenses } from "@/calculs/mouvements";
...
/** Ce dont le calcul des dépenses a besoin. */
export interface EntreesDepenses {
  baseTx: Transaction[];
  ...
  selType: string | null;
  selOrg: string | null;
  /** Le jeu COMPLET : les séries d'organismes ne doivent pas bouger au filtre. */
  allTransactions: Transaction[];
}

/**
 * Le calcul de l'écran Dépenses, sorti de React — lot C.3.
 *
 * Lot C.4 : les transferts internes sont les types de nature
 * `transfert-interne`, déclarés par la source.
 */
export function calculerDepenses(e: EntreesDepenses, regles: Regles) {
  const { baseTx, filteredTx, allMonthsInRange, selCat2, selType, selOrg, allTransactions } = e;
  ...
  const expByCat2 = ((): Cat2Slice[] => {
    const map: Record<string, number> = {};
    filteredTx
      .filter((t) => toucheDepenses(t, regles) && t.cat2 && t.cat2 !== "x")
      .forEach((t) => {
        map[t.cat2] = (map[t.cat2] || 0) + montantDepense(t, regles);
      });
  ...
  const expByType = ((): TypeSlice[] => {
    const map: Record<string, number> = {};
    filteredTx
      .filter((t) => toucheDepenses(t, regles) && t.type)
      .forEach((t) => {
        map[t.type] = (map[t.type] || 0) + montantDepense(t, regles);
      });
    return Object.entries(map)
      .map(([name, value]) => ({
  ...
  const detailRows = ((): DetailRow[] => {
    return filteredTx
      .filter((t) => toucheDepenses(t, regles))
      .map((t) => ({
        date: t.date,
        label: t.label,
        type: t.type,
        ...
        cat3: t.cat3,
        compte: t.compte,
        montant: montantDepense(t, regles),
      }));
  })();

  const detailTotal = ((() => detailRows.reduce((s, r) => s + r.montant, 0)
  ))();

  // ── 5. Top 10 marchands (Cat3) ────────────────────────────────────
  const topMerchants = ((): TopItem[] => {
    const map: Record<string, number> = {};
    filteredTx
      .filter((t) => toucheDepenses(t, regles) && t.cat3 && t.cat3 !== "x")
      .forEach((t) => {
        map[t.cat3] = (map[t.cat3] || 0) + montantDepense(t, regles);
      });
    return Object.entries(map)
      .map(([name, value]) => ({
  ...
  const compNvsN1 = ((): CompNvsN1Row[] => {
    ...
    const estTransfert = (type: string) => regles.aNature(type, "transfert-interne");
    const estDepense = (t: { dc: string; cat2: string; type: string }) =>
      toucheDepenses(t, regles) && !!t.cat2 && t.cat2 !== "x" && !estTransfert(t.type);
    ...
    const yPrev = allTransactions.filter(
      (t) =>
        ...
        (!selOrg || toOrganisme(t.compte, regles) === selOrg)
    );

    const mCur: Record<string, number> = {};
    const mPrev: Record<string, number> = {};
    yCur.forEach((t) => { mCur[t.cat2] = (mCur[t.cat2] || 0) + montantDepense(t, regles); });
    yPrev.forEach((t) => { mPrev[t.cat2] = (mPrev[t.cat2] || 0) + montantDepense(t, regles); });
  ...
  const compNvsN1Mois = ((() => allMonthsInRange.filter((mk) => mk.startsWith(curYear)).length
  ))();
