...
// `includes` : dans la démonstration, plusieurs comptes partagent le même
// organisme, donc le même début de libellé.

import { normaliserCle } from "@/services/lectureValeurs";
import type { Transaction } from "@/types";
import type { Regles } from "@/calculs/regles";
import { estRemboursement } from "@/calculs/mouvements";
  ...
  for (const mk of allMonths) {
    for (const t of txByMonth.get(mk) ?? []) {
      ...
      if (declare?.compteLie && !regles.aNature(t.type, "apport-exterieur")) {
        ...
        // C.7 — un remboursement (crédit de nature `remboursement`) ANNULE une
        // dépense partagée : sur le compte lié, il se répercute comme le débit
        // qu'il annule — sens Débit, montant négatif. Sans cela, rembourser une
        // dépense d'un compte joint ne reviendrait jamais sur le compte principal.
        const remb = estRemboursement(t, regles);
        const dcRepercute = remb ? "Débit" : t.dc;
        const mRepercute = remb ? -m : m;
        if (sens === "Les deux" || sens === dcRepercute) {
          const lie = regles.comptesDeclares.find((c) => c.id === declare.compteLie);
          if (lie && porteUnSolde.has(normaliserCle(lie.libelle))) {
            running[lie.libelle] -= mRepercute;
          }
        }
      }
