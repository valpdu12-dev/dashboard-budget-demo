...
import { montantDepense } from "@/calculs/mouvements";
...
export default function Depenses() {
  ...
  const kpis = useMemo(() => {
    ...
    const total = filteredTx.reduce((s, t) => s + montantDepense(t, regles), 0);
    const nb = debits.length;
    const moyMois = allMonthsInRange.length ? total / allMonthsInRange.length : 0;

    const curTotal = filteredTx
      .filter((t) => t.monthKey === currentMonth)
      .reduce((s, t) => s + montantDepense(t, regles), 0);
    const prevTotal = filteredTx
      .filter((t) => t.monthKey === prevMonth)
      .reduce((s, t) => s + montantDepense(t, regles), 0);

    return { total, nb, moyMois, curTotal, prevTotal };
  }, [filteredTx, allMonthsInRange, currentMonth, prevMonth, regles]);
