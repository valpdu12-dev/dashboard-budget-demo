...
import {
  Banknote, BadgeEuro, CalendarRange, Percent, Info,
} from "lucide-react";
...
export default function Salaire() {
  const { status, origin } = useDataStore();
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/*
        Lot E, décision E8 — la démonstration montre des employeurs et des
        salaires INVENTÉS. Sans cette mention, un lecteur pourrait les croire
        réels. Affichée seulement pour les données du site, jamais pour un
        fichier importé.
      */}
      {origin === "static" && (
        <div
          role="note"
          className="flex items-start gap-2 px-3.5 py-2.5 bg-indigo/[0.08] border border-indigo/25 rounded-lg text-[13px] text-indigo-text"
        >
          <Info size={16} className="shrink-0 mt-0.5" aria-hidden="true" />
          <span>
            Démonstration : employeurs et montants inventés.{" "}
            <span className="text-text-sec">
              Aucun salaire affiché ici n'est réel.
            </span>
          </span>
        </div>
      )}
