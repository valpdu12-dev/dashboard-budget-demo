...
export default function PretImmobilier() {
  ...
  const {
    status, hasData, pretDeclare, positionEstimee, controle, kpis, historyData, projectionData, donutData,
    dateFin, simulate,
  } = useMortgageData();
  ...
  const avancementPct = kpis ? Math.round(kpis.avancement * 1000) / 10 : 0;

  // L'historique mensuel est le seul graphique fondé uniquement sur les
  // transactions : il s'affiche même sans prêt déclaré (E.3).
  const histoCard = (
    <div className="card">
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/*
        E.3 — le prêt n'est plus jamais inventé. Sans bloc déclaré, cette page
        montre le seul historique réel des échéances, et dit ce qui lui manque
        pour la projection. Aucun chiffre affiché ne vient d'ailleurs que du
        fichier.
      */}
      {!pretDeclare && (
        <div
        ...
        >
          ...
          <span>
            Votre fichier ne déclare pas de prêt. Cette page montre l'historique
            réel de vos échéances, mais ni projection, ni capital restant, ni
            échéancier{" "}
            <span className="text-text-sec">
              — déclarez votre prêt (montant, date de début, taux annuel et
              durée) dans la feuille « Paramètres » pour les voir.
            </span>
          </span>
        </div>
      )}
      ...
      {/*
        F8 — un prêt sans date : sa position est déduite des intérêts du
        dernier mois. Elle est plausible, pas certaine — l'écran le dit.
      */}
      {pretDeclare && positionEstimee && (
        <div
          role="status"
          className="flex items-start gap-2 px-3.5 py-2.5 bg-amber/[0.08] border border-amber/25 rounded-lg text-[13px] text-amber"
        >
          <AlertTriangle size={16} className="shrink-0 mt-0.5" aria-hidden="true" />
          <span>
            Position estimée : votre fichier ne donne pas la date de votre prêt.
            L'échéance en cours est déduite des intérêts du dernier mois{" "}
            <span className="text-text-sec">
              — déclarez « Prêt — date de début » (le mois de la première
              échéance) dans la feuille « Paramètres » pour la calculer.
            </span>
          </span>
        </div>
      )}
      ...
      {/*
        F.2 — les transactions vérifient la déclaration. Un écart est dit,
        chiffré, avec ce qu'il faut vérifier ; une concordance aussi, pour
        qu'on sache que le contrôle a eu lieu.
      */}
      {pretDeclare && controle.etat === "diverge" && (
        <div
          role="alert"
          className="flex items-start gap-2 px-3.5 py-2.5 bg-amber/[0.08] border border-amber/25 rounded-lg text-[13px] text-amber"
        >
      ...
      {pretDeclare && controle.etat === "concorde" && (
        <p role="status" className="text-[12px] text-text-sec">
          Vos transactions concordent avec le prêt déclaré ({controle.moisCompares} mois
          comparés).
        </p>
      )}
      ...
      {/* Donut (si prêt déclaré) + Historique mensuel (toujours) */}
      {pretDeclare && kpis ? (
