...
// ═══════════════════════════════════════════════════════════════════════
//
// Deux listes, et une seule est écrite ici.
//
// LES NOMS PERSONNELS (établissements, employeurs, marchands, lieux,
// identité) ne sont PAS dans ce fichier — lot G. Les écrire ici, dans un dépôt
// public, les publiait. Ils viennent de scripts/listeNoire.mjs, qui les lit
// hors du dépôt (secret CI ou fichier privé ignoré). Un rapport ne cite
// jamais le texte trouvé, seulement le numéro du motif.
//
// LES MOTIFS DE SECRETS, eux, ne désignent personne : ils restent ici.

const lecture = lireListeNoire(RACINE);
const MOTIFS_PERSONNELS = lecture.motifs ?? [];
...
const PORTEURS_DE_LA_LISTE = new Map([
  [
    ...
    "le contrôle lui-même — il écrit les motifs de secrets qu'il cherche",
  ],
]);
...
const BUILD_AUTORISE = [
  ...
  // F4 — UN classeur, nommé. Son contenu est vérifié par `npm run modele`.
  { motif: /^modele\/Budget_modele\.xlsx$/, quoi: "classeur modèle (vérifié par npm run modele)" },
];
...
/**
 * F4 (lot F.4) — la SEULE exemption d'extension : un chemin exact, jamais
 * « les .xlsx ». Ce fichier échappe à la liste noire (c'est un binaire) ; il
 * n'est sûr que parce que `scripts/verifier-modele.mjs` le refait et le
 * compare à chaque `npm run check`.
 */
const EXEMPTION_XLSX = "public/modele/Budget_modele.xlsx";
...
const BINAIRE = new Set([".woff2", ".woff", ".ttf", ".png", ".jpg", ".ico", ".gz", ".tgz", ".xlsx"]);
...
function chercherInterdits(base, fichiers, etiquette, exempter = () => false) {
  for (const f of fichiers) {
    const texte = lireTexte(join(base, f));
    if (texte === null) continue;
    // Les noms personnels : AUCUNE exemption — plus aucun fichier du dépôt
    // n'a de raison de les porter. Rapport sans le texte trouvé.
    const personnel = trouverInterdit(texte, MOTIFS_PERSONNELS);
    if (personnel) {
      echecs.push(
        `[liste noire] ${etiquette} ${f}:${personnel.ligne} contient un nom interdit ` +
          `(motif n°${personnel.numero} de la liste privée)`
      );
      continue;
    }
    if (exempter(f)) continue;
...
// ── 0. La liste noire elle-même — lot G ────────────────────────────────
//
// Une liste absente, tronquée ou illisible ne laisse pas passer en silence.
if (lecture.erreurs.length) {
  if (lecture.motifs === null && listeFacultative()) {
    notes.push(
      "⚠️  liste noire ABSENTE, sautée par décision (LISTE_NOIRE_FACULTATIVE=1) : " +
        "aucun nom personnel n'a été cherché."
    );
  } else {
    for (const e of lecture.erreurs) echecs.push(`[liste noire] ${e}`);
  }
} else {
  notes.push(`liste noire : ${MOTIFS_PERSONNELS.length} motifs, depuis ${lecture.origine}`);
}

// G.4 — la liste privée ne doit jamais partir avec le dépôt.
//
// (a) plan/ doit rester ignoré par Git : c'est ce qui la garde hors du dépôt.
const gitignore = existsSync(join(RACINE, ".gitignore"))
  ? readFileSync(join(RACINE, ".gitignore"), "utf8").split(/\r?\n/).map((l) => l.trim())
  : [];
if (!gitignore.includes("plan/")) {
  echecs.push(
    "[liste noire] .gitignore n'ignore plus plan/ : le fichier privé " +
      `${CHEMIN_LISTE_PRIVEE} partirait avec le prochain commit.`
  );
}
// (b) En CI, le dépôt est un checkout : s'il contient le fichier privé,
// c'est qu'il a été versionné. Sur un poste, sa présence est normale.
if (process.env.CI && existsSync(join(RACINE, CHEMIN_LISTE_PRIVEE))) {
  echecs.push(
    `[liste noire] ${CHEMIN_LISTE_PRIVEE} est présent dans le dépôt récupéré par la CI : ` +
      "il a été versionné. Le retirer de l'historique."
  );
}
...
for (const f of fichiersSource) {
  ...
  } else if (f === EXEMPTION_XLSX) {
    notes.push(`exempté : ${f} — classeur modèle, refait et comparé par npm run modele`);
