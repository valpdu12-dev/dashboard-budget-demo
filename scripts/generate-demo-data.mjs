 ...
 *    ininterprétable.
 *
 * 2. SEULE LA STRUCTURE DE CONFIGURATION EST RÉELLE (lot E, E4 révisé au
 *    lot G). Comptes, natures, taux et comptes liés reprennent une
 *    configuration réelle, RECOPIÉE À LA MAIN, sous des NOMS GÉNÉRIQUES
 *    (Banque A, Banque B…). Aucun nom réel n'entre ici. Tout le reste
 *    est inventé : montants, dates, libellés, marchands, villes et
 *    employeurs. Rien n'a été copié, transformé ni échantillonné depuis un
 *    classeur réel — ce script n'importe que de quoi ÉCRIRE, il ne peut lire
 *    aucun fichier.
 *
...
// SORTIE_DEMO permet de rediriger l'écriture vers un dossier temporaire —
// le garde-fou de reproductibilité (E.3) s'en sert pour régénérer sans
// toucher public/data/. Sans la variable, on écrit dans public/data/.
const SORTIE = process.env.SORTIE_DEMO || join(ICI, "..", "public", "data");
...
/**
 * Le compte d'où sortent les sorties d'épargne — lot F.3.
 *
 * Avant, la colonne Compte portait « Sortie Epargne » : un libellé de TYPE
 * écrit dans la colonne des comptes (D2). Le modèle téléchargeable s'en
 * plaignait à l'import. C'est désormais un compte déclaré, qui ne porte pas
 * de solde : l'argent sort d'une épargne que la démo ne suit pas, et arrive
 * sur le compte principal (« Compte crédité par les sorties d'épargne »).
 */
const COMPTE_SORTIE_EPARGNE = "Épargne hors suivi";
...
// Employeurs INVENTÉS (E5) : des cabinets fictifs, aucune vraie entreprise.
// Ils n'apparaissent que dans l'historique de paie de la démonstration.
const EMPLOYEURS = ["Cabinet Vorentis", "Groupe Aldruvie", "Kessante Ingénierie", "Ternovex Conseil", "Malderim Systèmes"];

// ═══════════════════════════════════════════════════════════════════════
// 4. PRÊT IMMOBILIER (fictif, cohérent)
// ═══════════════════════════════════════════════════════════════════════

// Lot F.3 (décision O6) — le prêt se déclare en forme « taux » : montant,
// date de début, taux annuel, durée. La mensualité se CALCULE, comme dans
// l'application (`mensualiteDepuisTaux`) : 893,64 € au lieu des 893,60 €
// d'avant, qui correspondaient à un taux de 1,7995 %.
const PRET = {
  montant: 180000,
  tauxAnnuel: 0.018,
  echeances: 240,
...
{
  const r = PRET.tauxAnnuel / 12;
  PRET.mensualite =
    Math.round(((PRET.montant * r) / (1 - Math.pow(1 + r, -PRET.echeances))) * 100) / 100;
}
...
for (const mk of MOIS_TX) {
  ...
  if (!maigre) {
    tx({
      compte: COMPTE_PRINCIPAL, type: "Epargne Banque A", date: dateDans(mk, 26),
      montant: 150, cat2: "Comptes Bancaires", dc: "Débit", label: "Virement épargne",
    });
    tx({
      compte: COMPTE_PRINCIPAL, type: "Epargne Banque C", date: dateDans(mk, 26),
    ...
    if (parfois(0.3)) {
      tx({
        compte: COMPTE_PRINCIPAL, type: "Epargne Banque B", date: dateDans(mk, entier(10, 25)),
...
const paie = MOIS_PAIE.map((mk) => {
  ...
  // Lot F.3 — le net retire aussi les « autres retenues », comme le lecteur
  // du format (brut − cotisations + indemnités − autres retenues). Avant, le
  // jeu tirait des retenues sans les retirer : un fichier refait depuis la
  // démo donnait un autre net. Tirage au même rang : rien d'autre ne bouge.
  const retenues = euros(entre(0, 42));
  return { mk, entreprise: poste.entreprise, brut, net: euros(net + indem - retenues), cotSal, indem, retenues };
});
...
/** Les types de dépense du jeu, avec leur classe — relevés, jamais inventés. */
function typesDeDepense() {
  const classes = new Map();
  for (const t of transactions) {
    if (t.dc !== "Débit" || !t.cat1) continue;
    const vue = classes.get(t.type);
    if (vue !== undefined && vue !== t.cat1) {
      throw new Error(`Le type « ${t.type} » porte deux classes : ${vue} et ${t.cat1}.`);
    }
    classes.set(t.type, t.cat1);
  }
  return [...classes.keys()]
    .filter((l) => l !== "Intérêt du prêt")
    .sort((a, b) => a.localeCompare(b, "fr"))
    .map((l) => type(l, [], classes.get(l)));
}
...
function parametrageDemo() {
  return {
    ...
    comptes: [
      ...
      compte(COMPTE_SORTIE_EPARGNE, {
        couleur: "#94a3b8", porteUnSolde: false,
      }),
    ],
    types: [
      ...
      type("Epargne Banque A", ["epargne", "transfert-interne"]),
      type("Epargne Banque B", ["epargne"]),
      type("Epargne Banque C", ["epargne"]),
      ...
      // Lot F.3 (F11 amendé) — tous les types de DÉPENSE sont déclarés, avec
      // leur classe. Un crédit sur l'un d'eux est un remboursement, et le
      // modèle téléchargeable porte la même table : une seule vérité. Les
      // classes sont relevées sur les transactions générées ci-dessus, pas
      // écrites une seconde fois à la main.
      ...typesDeDepense(),
    ],
...
const config = {
  ...
  // O6 : la forme « taux ». `mensualite` et `echeances` restent écrits, car
  // `Config.pret` les exige : ce sont les valeurs CALCULÉES, que le lecteur
  // produirait lui-même depuis un fichier en forme « taux ».
  pret: {
    montant: PRET.montant,
    mensualite: PRET.mensualite,
    echeances: PRET.echeances,
    taux_annuel: PRET.tauxAnnuel,
    date_debut: PRET.premiereEcheance,
  },
...
console.log(`Prêt           ${PRET.montant} € à ${(PRET.tauxAnnuel * 100).toFixed(2)} % sur ${PRET.echeances} mois, 1re en ${PRET.premiereEcheance} → ${PRET.mensualite} €/mois`);
